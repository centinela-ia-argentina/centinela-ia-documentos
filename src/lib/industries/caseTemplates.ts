...
export const caseTemplatesByIndustryAndType: Record<string, Record<string, CaseTemplate>> = {
  legal: {
    'Caso jurídico': {
      checklist: [
        'Poder / carta poder',
  ...
  general: {
    'General': fallbackTemplate,
    'Otro': fallbackTemplate,
  },
  contable: {
    'Carpeta contable mensual': fallbackTemplate,
    'Cliente': fallbackTemplate,
    'Proveedor': fallbackTemplate,
    'Otro': fallbackTemplate,
  }
};

export function getCaseTemplate(industry: IndustryType, caseType?: string | null): CaseTemplate | null {
  if (!caseType) return null;
  const industryTemplates = caseTemplatesByIndustryAndType[industry];
  if (!industryTemplates) return null;
  return industryTemplates[caseType] || null;
}
