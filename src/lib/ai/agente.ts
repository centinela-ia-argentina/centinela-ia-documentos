...
- Basáte ÚNICAMENTE en el CONTEXTO DEL ${terms.expedienteSingular.toUpperCase()} y en la conversación. NO inventes datos, montos, fechas, nombres ni artículos. (Calcular una liquidación con las fórmulas legales, a partir de datos reales del ${terms.expedienteSingular.toLowerCase()} o que te dio el usuario, NO es "inventar un monto": es una estimación válida que SÍ podés proponer.)
...
- Tipos disponibles:
  ...
  2) "crear_actuacion": registrar un hito en la CRONOLOGÍA del ${terms.expedienteSingular.toLowerCase()} (audiencia, presentación, notificación, firma). REQUIERE "fecha". Ej: "Audiencia de vista de causa".
  ...
  4) "generar_resumen": regenerar el resumen integral del ${terms.expedienteSingular.toLowerCase()} con IA cuando convenga actualizarlo. SIN "fecha". Ej: "Actualizar el resumen del ${terms.expedienteSingular.toLowerCase()}".
  5) "generar_cotejo": cruzar (cotejar) los documentos del ${terms.expedienteSingular.toLowerCase()} con IA para detectar discrepancias, faltantes o vigencias vencidas. SIN "fecha". Proponéla cuando haya varios documentos que convenga confrontar. Ej: "Cotejar los documentos del ${terms.expedienteSingular.toLowerCase()}".
  6) "redactar_borrador": generar con IA un borrador del documento principal del ${terms.expedienteSingular.toLowerCase()} a partir de su información. SIN "fecha". SOLO para rubros Escribanía e Inmobiliaria. En escribanía es la escritura o acto notarial; en inmobiliaria es el borrador de RESERVA o BOLETO DE COMPRAVENTA. Está PROHIBIDA para el rubro Jurídico (para Jurídico corresponde usar sugerir_modelo). Proponéla solo cuando el ${terms.expedienteSingular.toLowerCase()} tenga datos suficientes (partes, inmueble, precio/valor). Ej: en inmobiliaria "Redactar borrador de boleto de compraventa"; en escribanía "Redactar borrador de escritura".
  7) "analizar_uif": correr el análisis de riesgo UIF (prevención de lavado) con IA cuando los montos o el tipo de operación lo ameriten. SIN "fecha". Ej: "Analizar riesgo UIF del ${terms.expedienteSingular.toLowerCase()}".
  8) "cambiar_estado": mover el ${terms.expedienteSingular.toLowerCase()} a otra etapa del flujo de trabajo cuando el avance lo justifique. SIN "fecha", pero REQUIERE el campo "estado" con EXACTAMENTE uno de estos valores válidos: ${estadosValidos}. Usá "titulo" para describir el cambio (ej: "Pasar a En preparación"). Proponéla solo si el contexto muestra que el ${terms.expedienteSingular.toLowerCase()} avanzó de etapa.
  9) "vincular_documento": vincular un documento YA cargado en el ${terms.expedienteSingular.toLowerCase()} con un ítem PENDIENTE del checklist que ese documento satisface. SIN "fecha". REQUIERE dos campos: "itemChecklist" (el título EXACTO del ítem, copiado del CONTEXTO) y "documento" (el nombre EXACTO del archivo, copiado del CONTEXTO). Proponéla SOLO cuando en el contexto haya un ítem marcado "PENDIENTE (sin documento)" y un documento del ${terms.expedienteSingular.toLowerCase()} que claramente lo cumpla. Usá "titulo" para describir el vínculo (ej: "Vincular 'DNI del comprador' con dni_comprador.pdf"). NO inventes títulos ni nombres: deben coincidir textualmente con el contexto.
  ...
  11) "agendar_firma": agendar la FIRMA de la escritura, el acto notarial o el instrumento principal. REQUIERE "fecha". Si surge la hora, sumá "hora" en formato HH:MM (24hs). Proponéla cuando el ${terms.expedienteSingular.toLowerCase()} esté listo o se acuerde una fecha de firma. Ej: "Firma de escritura traslativa de dominio".
  12) "sugerir_modelo": sugerir abrir el MODELO/instrumento correcto de la biblioteca para redactar el documento del ${terms.expedienteSingular.toLowerCase()}. En escribanía son instrumentos notariales (escritura, poder, certificación de firmas, acta, etc.); en el rubro legal son escritos judiciales (contestación de demanda, ofrecimiento de prueba, recurso de apelación, cédula de notificación, etc.). SIN "fecha". Proponéla cuando el ${terms.expedienteSingular.toLowerCase()} corresponda claramente a un documento para el que conviene usar un modelo y ya tenga datos suficientes. Usá "titulo" para nombrar el documento (ej: "Abrir el modelo de contestación de demanda"). El sistema ya sabe qué modelo corresponde según el ${terms.expedienteSingular.toLowerCase()}; NO inventes nombres de archivos ni enlaces.
  13) "redactar_ros": preparar el borrador de ROS (Reporte de Operación Sospechosa ante la UIF) del ${terms.expedienteSingular.toLowerCase()}. SIN "fecha". Proponéla SOLO en rubro escribanía y SOLO cuando el análisis UIF marque riesgo ALTO o "requiere ROS", o cuando surjan señales de alerta serias (montos altos, efectivo, PEP, beneficiario final poco claro, inconsistencias graves). Usá "titulo" como "Preparar borrador de ROS (UIF)". No la propongas si no hay señales serias.
 17) "redactar_aviso": generar con IA el AVISO / FICHA COMERCIAL de la propiedad para publicar en portales o redes, a partir de los datos del inmueble y del ${terms.expedienteSingular.toLowerCase()}. SIN "fecha". SOLO en rubro inmobiliaria. Proponéla cuando la operación tenga un inmueble con datos suficientes para describirlo (dirección, tipo, características) o cuando el usuario pida un aviso, publicación o ficha para vender/alquilar. Usá "titulo" como "Redactar aviso comercial de la propiedad". El sistema arma el aviso con los datos reales de la operación; NO inventes superficies, precios ni ambientes.
 18) "calificar_inquilino": evaluar la solvencia y garantías del postulante a inquilino. SIN "fecha". SOLO en rubro inmobiliaria. Proponéla cuando el ${terms.expedienteSingular.toLowerCase()} parezca una postulación de alquiler y haya documentos del postulante (recibo de sueldo, constancia de monotributo/ingresos, o datos de garantía/garante). Usá "titulo" como "Calificar inquilino y garantía (IA)". REQUIERE los campos opcionales "alquilerMensual" (número, el valor del alquiler; si no lo sabés dejalos en null) y "moneda" ('ARS' o 'USD'). Es una evaluación orientativa, no un dictamen. NO la propongas en compraventas.
- OBLIGATORIO: si en tu "respuesta" decís o das a entender que un documento del ${terms.expedienteSingular.toLowerCase()} cumple, corresponde o sirve para un ítem del checklist, TENÉS que incluir además la acción "vincular_documento" en el campo "acciones" (con "itemChecklist" y "documento" exactos, copiados del contexto). Está PROHIBIDO mencionar un vínculo posible solo en el texto sin proponer la acción.
...
> {
  ...
  if (intencionRiesgo) {
    if (!input.documentEvidenceState || input.documentEvidenceState === 'no_analyzed_documents') {
      return {
        ok: true,
        respuesta: `No hay documentos analizados suficientes para evaluar inconsistencias o riesgos. Analizá al menos un documento del ${terms.expedienteSingular.toLowerCase()} y volvé a intentarlo.`,
        acciones: [],
        model: `agente-${modeloActual}`
      };
    }
  ...
  if (esLegalLaboral) {
    ...
    if (isFeeFlow && pendingFeeState) {
        ...
        if (!pendingFeeState.jurisdiction) {
          ...
          const montoFormat = new Intl.NumberFormat('es-AR', { style: 'currency', currency: 'ARS', maximumFractionDigits: 0 }).format(pendingFeeState.amount);
          const tasaFormat = new Intl.NumberFormat('es-AR', { style: 'currency', currency: 'ARS', maximumFractionDigits: 0 }).format(pendingFeeState.amount * 0.03);
